USE [Inmuebles]
GO
/****** Object:  StoredProcedure [dbo].[SmartDefragmentation_SP]    Script Date: 9/19/2018 10:05:04 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[SmartDefragmentation_SP]
AS
BEGIN TRY
SET NOCOUNT ON
...
SET @JobName = 'DBA - SmartDefragmentation INMUEBLES'
...
-- Exceptuo EmailEnvio porque es una tabla de staging
SELECT
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				SET @cmd = 'UPDATE STATISTICS ['+@SchemaName+'].['+@TableName+'] ['+@IndexName +'] WITH SAMPLE 30 PERCENT'
	...
	BEGIN
		...
		EXEC master.dbo.InsertJobHistory_SP @JobName=@JobName, @Output = @Output
		SET @cmd = 'ALTER INDEX ['+@IndexName+'] ON ['+@SchemaName+'].['+@TableName+'] REBUILD'
		-- Se comentan estas lineas ya que Standard No soporta REBUILD ONLINE
		-- Si no posee un tipo de Dato LOB y no tiene ninguna columna del tipo text, image o ntext fuerzo a lo que haga ONLINE.
		IF @alloc_unit_type_desc <> 'LOB_DATA' AND NOT EXISTS (SELECT COUNT(*) FROM sys.columns WHERE user_type_id in (34,35,99) AND OBJECT_NAME(OBJECT_ID) = @TableName)
